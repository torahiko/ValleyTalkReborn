...
namespace ValleytalkReborn
{
    ...
    public static class Util
    {
        ...
        /// <summary>
        /// 专用于场景感知的村民检测（Prompts、Bark 等场景）。
        /// 室外 10 格宽视野，室内 7 格防穿墙，异常默认兜底为室外。
        /// </summary>
        public static IEnumerable<NPC> GetSceneVillagers(NPC npc)
        {
            var location = npc?.currentLocation ?? Game1.currentLocation;
            if (npc == null || location?.characters == null)
                return Enumerable.Empty<NPC>();

            // 室内 7 格，室外 10 格，兜底默认室外
            bool isOutdoors = location?.IsOutdoors ?? true;
            float radiusTiles = isOutdoors ? 10f : 7f;
            float maxDistanceSquared = radiusTiles * radiusTiles;

            Vector2 speakerLocation = npc.Tile;
            string speakerName = npc.Name;

            var nearbyNpcs = new List<NPC>();
            foreach (var otherNpc in location.characters)
            {
                if (otherNpc == null || otherNpc.Name == speakerName || !otherNpc.CanReceiveGifts())
                    continue;
