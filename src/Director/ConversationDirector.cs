...
public sealed class ConversationDirector : IConversationDirector
{
        ...
        DialogueContext context, Character character, InstructionsBranch branch, Prompts prompts)
    {
        ...
        // SpouseWaiting：探测 + 构建
        if (SpouseWaitingEvent.HasPendingSpouseDialogue(character.Name))
        {
        ...
        if (ModEntry.Config.EnableEmotionSystem && !npcIsNull)
        {
            try
            {
        ...
        // PlayerProfile：Greeting 用 ""，其余用末条玩家台词。
